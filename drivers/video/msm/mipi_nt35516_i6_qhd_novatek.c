...
#define NT35516_SLEEP_OFF_DELAY 10
#define NT35516_DISPLAY_ON_DELAY 0
...
static struct dsi_cmd_desc nt35516_cmd_display_on_cmds_rotate[] = {
...
static struct dsi_cmd_desc nt35516_video_display_on_cmds[] = {
	...
	{DTYPE_DCS_WRITE, 1, 0, 0, NT35516_SLEEP_OFF_DELAY, sizeof(exit_sleep),
			exit_sleep},
	{DTYPE_DCS_WRITE, 1, 0, 0, NT35516_DISPLAY_ON_DELAY, sizeof(display_on),
			display_on},
};

static struct dsi_cmd_desc nt35516_video_display_on_cmds_rotate[] = {
	{DTYPE_DCS_LWRITE, 1, 0, 0, 10,
...
static int mipi_novatek_nt35516_qhd_lcd_on(struct platform_device *pdev)
{
	...
	if (mipi_nt35516_pdata && mipi_nt35516_pdata->rotate_panel)
		rotate = mipi_nt35516_pdata->rotate_panel();
	...
	if (mipi->mode == DSI_VIDEO_MODE) {
		...
		if (rotate) {
			mipi_dsi_cmds_tx(&nt35516_tx_buf,
				nt35516_video_display_on_cmds_rotate,
			ARRAY_SIZE(nt35516_video_display_on_cmds_rotate));
		}
		...
		if (rotate) {
			mipi_dsi_cmds_tx(&nt35516_tx_buf,
				nt35516_cmd_display_on_cmds_rotate,
			ARRAY_SIZE(nt35516_cmd_display_on_cmds_rotate));
		}
	}
