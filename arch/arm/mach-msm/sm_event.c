...
{
	if (event_ops && event_ops->sm_set_event_mask) {
		atomic_inc (&event_use_count);
		if (enable_flag)
			event_ops->sm_set_event_mask(event_mask);
		atomic_dec (&event_use_count);
	}
}
...
{
	if (event_ops && event_ops->sm_get_event_mask) {
		uint32_t rc = 0;
		atomic_inc (&event_use_count);
		if (enable_flag)
			rc = event_ops->sm_get_event_mask();
		atomic_dec (&event_use_count);
		return rc;
	}
